...
Массив — один из [типов языка TRIK Studio](/docs/studio/programming-visual/expression-syntax/types/), хранящий набор значений (элементов массива), идентифицируемых по индексу. Нумерация индексов в массиве начинается с 0.
...
Сделано это для того, чтобы интерпретировать перечисления значений (например, порты моторов в блоке [«Моторы вперёд»](/docs/trik/programming-visual/blocks/)) как массивы. Поэтому везде, где используется запись значений через запятую, можно использовать массив.
