...
:::warning
**Внимание!** Редактирование свойств блока [«Подпрограмма»](/studio/programming-visual/blocks/#subprogram) осуществляется только с помощью контекстного меню.
:::

## Редактирование свойств с помощью панели &#123;#panel&#125;
При выделении блока, связи, или элемента, на панели [«Редактор свойств»](/studio/interface/#panel-property-editor) отображаются все свойства текущего элемента.
...
Для свойств, которые допускают только определенный набор значений, например, свойство «Порт» у блока [«Ждать ИК датчик расстояния»](/trik/programming-visual/blocks/#wait-for-infrared-distance), при попытке их редактирования, появится выпадающее меню, где выбирается нужное значение:
