...
Контроллеры ТРИК могут взаимодействовать друг с другом, что позволяет решать [задачи](/trik/wi-fi/interaction/interaction-example/) по передаче сообщений между роботами.
...
1\. Выберите **ведущий** контроллер и подключите его к существующей Wi-Fi-сети в [режиме клиента](/trik/wi-fi/network-connection/#client).

2\. В [веб-интерфейсе](/trik/web-interface/) **ведущего** контроллера в поле «Взаимодействие роботов» укажите бортовой номер (обычно для ведущего это `01`, но можно любой другой) и его IP-адрес.

![Пример ввода бортномера и IP-адреса ведущего контроллера](/gitbook/assets/configurator-main.png)

3\. Подключите **все** остальные контроллеры к этой же Wi-Fi-сети в [режиме клиента](/trik/wi-fi/network-connection/#client).
...
1\. Выберите **ведущий** контроллер и раздайте с него Wi-Fi с помощью [режима точки доступа](/trik/wi-fi/network-connection/#accesspoint) (выберите в меню `Сеть)`.
...
4\. Подключите остальные контроллеры к ведущему с помощью [режима Wi-Fi-клиент](/trik/wi-fi/network-connection/#client). Для этого в веб-интерфейсе каждого контроллера необходимо в полях «Wi-Fi клиент» указать имя и пароль сети ведущего контроллера в режиме Wi-Fi точки доступа.
...
[Пример решения задачи по взаимодействию двух роботов](/trik/wi-fi/interaction/interaction-example/)
