---
title: "Подключение контроллера ТРИК к компьютеру и смартфону"
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Подключение контроллера ТРИК к компьютеру и смартфону

Контроллер ТРИК может быть подключен к другим устройствам двумя способами:

1. Используем контроллер ТРИК как [**точку доступа**](#accesspoint) \*\*(\*\*то есть контроллер сам создает новую сеть) и подключаем к нему устройства.
2. Используем контроллер ТРИК в режиме [**Wi-Fi-клиента**](#client) и подключаем устройства и сам контроллер к существующей сети.

## Способ 1. Контроллер в режиме точки доступа Wi-Fi &#123;#accesspoint&#125;
В этом режиме контроллер сам создает новую сеть, и мы к нему подключаем устройства.

1\. Зайдите в меню `Сеть`. Контроллер раздаст Wi-Fi, а на экране появятся параметры сети:

* Имя сети.
* Пароль сети.
* IP-адрес контроллера.

![](/gitbook/assets/dac.png)

2\. При подключении к сети Wi-Fi компьютера или смартфона выберите сеть контроллера и введите пароль.

## Способ 2. Контроллер в режиме Wi-Fi-клиента &#123;#client&#125;
В этом режиме контроллер и устройства мы подключаем к уже существующей Wi-Fi-сети.

Для этого нам в [веб-интерфейсе](/docs/trik/web-interface/) контроллера надо прописать имя и пароль сети, к которой будем подключать контроллер.

1\. Для запуска веб-интерфейса подключите компьютер или смартфон к контроллеру как к [точке доступа](#accesspoint).

2\. В адресной строке браузера введите IP-адрес контроллера. Откроется веб-интерфейс.

<Tabs>
<TabItem value={"на-ноутбуке"} label={"На ноутбуке"}>
![](/gitbook/assets/configurator.png)
</TabItem>

<TabItem value={"на-смартфоне"} label={"На смартфоне"}>
![](/gitbook/assets/configurator-mobile.jpg)
</TabItem>
</Tabs>

3\. В поле «Wi-Fi-клиент» введите имя и пароль сети, к которой хотите подключить контроллер. Нажмите кнопку «Сохранить».

4\. Переключите контроллер в режим Wi-Fi-клиента.

![](/gitbook/assets/dac%20%281%29.png)

5\. При успешном подключении вверху экрана рядом с иконкой <img src="/gitbook/assets/icon-1.png" alt="" data-size="line" /> отобразится ip-адрес контроллера, а в списке иконкой <img src="/gitbook/assets/icon-2.png" alt="" data-size="line" /> будет выделено имя сети.

![](/gitbook/assets/screenshot_5.png)


{/* Compatibility anchors for old GitBook/Docusaurus links. */}
###### ​ {#accesspoint}
###### ​ {#client}
