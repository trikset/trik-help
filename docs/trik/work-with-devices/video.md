...
Для того чтобы включить камеру как сенсор линии, вам необходимо изменить режим работы камеры в свойствах блока [«Включить видеокамеру»](/docs/trik/programming-visual/blocks/#initialize-videocamera), на `Сенсор линии`. Режим `Сенсор линии` имеет сенсорную переменную [lineSensor](/docs/trik/programming-visual/sensory-variables/#linesensor).&#x20;
...
:::warning
**Внимание!** Блок [«Ждать нажатия кнопки»](/docs/trik/programming-visual/blocks/#wait-for-button) необходим, чтобы камера успела включиться. Когда на экране робота появится изображение с камеры, можно приступать к следующему пункту.
:::
...
4\. Напишите регулятор для движения. Используйте блок [«Датчик линии в переменную»](/docs/trik/programming-visual/blocks/#line-detector-into-variable), который записывает показания отклонения объекта от центра экрана по оси `Х` (от -100 до 100) в переменную, указанную в свойствах. Другими словами, это динамическая ошибка.
...
Камера как сенсор объекта необходима для решения задач распознавания и отслеживания объектов. Для того чтобы включить камеру как сенсор линии, вам необходимо изменить режим работы камеры, в свойствах блока [«Включить видеокамеру»](/docs/trik/programming-visual/blocks/#initialize-videocamera), на `Сенсор объекта`.\
Режим `Сенсор объекта` имеет сенсорные переменные: [objectSensorSize, objectSensorX, objectSensorY.](/docs/trik/programming-visual/sensory-variables/)
...
5\. Далее добавьте вывод в консоль переменных `a`, `b`, `c` и закончите цикл блоком [«Таймер»](/docs/studio/programming-visual/blocks/#timer):
...
Для того чтобы включить камеру как сенсор линии, вам необходимо изменить режим работы камеры, в свойствах блока [«Включить видеокамеру»](/docs/trik/programming-visual/blocks/#initialize-videocamera), на `Сенсор цвета`.

Режим `Сенсор цвета` имеет сенсорные переменные трех цветов (красный, зеленый, синий): [colorSensorR, colorSensorG, colorSensorB](/docs/trik/programming-visual/sensory-variables/#colorsensor).
...
:::info
**Внимание!** Блок [«Ждать нажатия кнопки»](/docs/trik/programming-visual/blocks/#wait-for-button) необходим, чтобы камера успела включиться. Когда на экране робота включиться изображение с камеры, можно приступать к следующему пункту.
:::
